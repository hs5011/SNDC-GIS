...
const GeneralStatusForm: React.FC<GeneralStatusFormProps> = ({ initialData, onSubmit, onClose }) => {
  ...
  const handleSave = () => {
    if (!formData.code?.trim() || !formData.name?.trim()) {
      alert("CẢNH BÁO: Vui lòng nhập đầy đủ Mã và Tên tình trạng!");
      return;
    }
    onSubmit(formData);
  };
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col border border-slate-200">
        ...
        <div className="p-6 space-y-4">
          <div className="space-y-2">
            <label className="text-sm font-bold text-slate-700">Mã tình trạng <span className="text-red-500">*</span></label>
            <input 
              value={formData.code} 
          ...
          <div className="space-y-2">
            <label className="text-sm font-bold text-slate-700">Tên tình trạng <span className="text-red-500">*</span></label>
            <input 
              value={formData.name} 
          ...
          <div className="pt-4 flex justify-end gap-3 border-t mt-4">
            ...
            <button onClick={handleSave} className="px-6 py-2 bg-blue-600 text-white font-black rounded-xl flex items-center gap-2 shadow-lg transform active:scale-95 transition-all">
