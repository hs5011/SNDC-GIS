...
import { X, Save, Search, CheckCircle2, Building, Plus, Trash2, Award, List, UserCog, Edit, RotateCcw } from 'lucide-react';
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[95vh] overflow-hidden flex flex-col border border-slate-200">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar">
          <div className="space-y-4">
                  ...
                  <div className="bg-white border rounded-xl shadow-xl overflow-hidden divide-y max-h-[350px] overflow-y-auto custom-scrollbar">
                    {filteredHouses.map(h => (
                       <button key={h.id} onClick={() => { setSelectedHouseId(h.id); setHouseSearch(''); }} className="w-full text-left px-5 py-4 hover:bg-amber-50 transition-colors flex justify-between items-center group">
                         ...
                         <div className="px-3 py-1 bg-amber-100 text-amber-700 text-[10px] font-black rounded-lg opacity-0 group-hover:opacity-100 transition-opacity uppercase">Chọn căn này</div>
                       </button>
                    ))}
                  </div>
                )}
            ...
            {selectedHouse ? (
               <div className="border-2 border-amber-500 rounded-2xl p-5 flex justify-between items-center bg-amber-50 shadow-md">
                 <div className="flex items-center gap-4">
                   <div className="w-12 h-12 bg-amber-600 rounded-xl flex items-center justify-center text-white shadow-lg"><Building size={24} /></div>
                   <div>
                          ...
                          <tr key={idx} className={`hover:bg-amber-50/30 transition-colors ${editingTempIndex === idx ? 'bg-amber-50/50' : ''}`}>
                            ...
                            <td className="px-6 py-3 text-right">
                              <div className="flex justify-end gap-1">
                                <button onClick={() => { setEditingTempIndex(idx); setCurrentMedal(medalsList[idx]); }} className="p-1.5 text-blue-600 hover:bg-blue-100 rounded-lg transition-colors"><Edit size={14} /></button>
                                <button onClick={() => setMedalsList(prev => prev.filter((_, i) => i !== idx))} className="p-1.5 text-red-500 hover:bg-red-100 rounded-lg transition-colors"><Trash2 size={14} /></button>
                              </div>
                            </td>
                          </tr>
                        ))}
        ...
        <div className="p-6 border-t bg-slate-50 flex justify-end gap-3 shrink-0">
          ...
          <button onClick={handleSaveAll} className="px-10 py-3 bg-amber-600 text-white font-black rounded-xl shadow-xl transform active:scale-95 hover:bg-amber-700 transition-all flex items-center gap-2 text-sm">
