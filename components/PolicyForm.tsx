...
const PolicyForm: React.FC<PolicyFormProps> = ({ 
  ...
  const handleSaveAll = () => {
    ...
    if (isEditing) {
      if (!currentPolicy.HoTen?.trim() || !currentPolicy.LoaiDienChinhSach) { alert("CẢNH BÁO: Thiếu thông tin bắt buộc!"); return; }
      onSubmit([{ ...currentPolicy, LinkedHouseId: selectedHouseId }]);
    } else {
      if (policiesList.length === 0) { alert("CẢNH BÁO: Vui lòng Thêm hồ sơ vào danh sách trước khi Lưu!"); return; }
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[95vh] overflow-hidden flex flex-col border border-slate-200">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar">
          <div className="space-y-4">
            ...
            {selectedHouse ? (
              <div className="border rounded-xl p-4 flex justify-between items-center bg-indigo-50 border-indigo-100 shadow-sm">
                ...
                {!isEditing && !isHouseLocked && <button onClick={() => setSelectedHouseId(undefined)} className="text-red-500 hover:bg-red-50 p-2 rounded-lg transition-colors"><Trash2 size={16} /></button>}
              </div>
            ) : <div className="border-2 border-dashed border-slate-200 p-6 text-center text-slate-400 text-xs font-bold bg-slate-50/50 italic">Bắt buộc nhập từ khóa và chọn số nhà liên kết hồ sơ chính sách</div>}
          </div>
            ...
            <div className="space-y-6 border-t pt-6">
              ...
              <div className="p-6 rounded-2xl border bg-slate-50/50 border-slate-200 space-y-4 shadow-inner">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-1"><label className="text-xs font-bold text-slate-600">Họ và tên <span className="text-red-500 font-black">*</span></label><input value={currentPolicy.HoTen || ''} onChange={e => setCurrentPolicy({...currentPolicy, HoTen: e.target.value})} className="w-full px-3 py-2 border rounded-lg text-sm bg-white" placeholder="Họ và tên" /></div>
                  <div className="space-y-1"><label className="text-xs font-bold text-slate-600">Diện chính sách <span className="text-red-500 font-black">*</span></label><select value={currentPolicy.LoaiDienChinhSach || ''} onChange={e => setCurrentPolicy({...currentPolicy, LoaiDienChinhSach: e.target.value})} className="w-full px-3 py-2 border rounded-lg text-sm bg-white"><option value="">-- Chọn diện --</option>{policyTypes.map(pt => <option key={pt.id} value={pt.name}>{pt.name}</option>)}</select></div>
                  <div className="space-y-1"><label className="text-xs font-bold text-slate-600">Tỷ lệ tổn thương</label><input value={currentPolicy.TyLeTonThuong || ''} onChange={e => setCurrentPolicy({...currentPolicy, TyLeTonThuong: e.target.value})} className="w-full px-3 py-2 border rounded-lg text-sm bg-white" placeholder="VD: 81%" /></div>
                  <div className="space-y-1"><label className="text-xs font-bold text-slate-600">Mức trợ cấp</label><input type="number" value={currentPolicy.SoTien || 0} onChange={e => setCurrentPolicy({...currentPolicy, SoTien: parseFloat(e.target.value) || 0})} className="w-full px-3 py-2 border rounded-lg text-sm font-bold text-emerald-600 bg-white" /></div>
                  <div className="space-y-1"><label className="text-xs font-bold text-slate-600">Người nhận thay</label><select value={currentPolicy.NguoiNhanThay || ''} onChange={e => setCurrentPolicy({...currentPolicy, NguoiNhanThay: e.target.value})} className="w-full px-3 py-2 border rounded-lg text-sm bg-white"><option value="">-- Chính chủ --</option>{availableReceivers.map(rec => <option key={rec.id} value={rec.name}>{rec.name}</option>)}</select></div>
                </div>
                {!isEditing && (
        ...
        <div className="p-6 border-t bg-slate-50 flex justify-end gap-3 shrink-0">
          ...
          <button onClick={handleSaveAll} className="px-8 py-2 bg-indigo-600 text-white font-black rounded-xl shadow-lg transform active:scale-95 hover:bg-indigo-700 transition-all flex items-center gap-2"><Save size={18} /> Lưu hồ sơ</button>
        </div>
      </div>
    </div>
  );
