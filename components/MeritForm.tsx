...
const MeritForm: React.FC<MeritFormProps> = ({ 
  ...
  const availableReceivers = selectedHouse ? [
  ...
  const handleAddToList = () => {
    ...
    if (!isEditing) {
      setCurrentMerit({
        HoTen: '', QuanHe: '', LoaiDoiTuong: '', SoQuanLyHS: '', SoTien: 0,
        NguoiNhanThay: '', GhiChu: '', HinhThucNhan: 'Tiền mặt'
      });
    }
  };
  ...
  const removeFromList = (index: number) => {
    if (editingTempIndex === index) {
      setEditingTempIndex(null);
      setCurrentMerit({ HoTen: '', QuanHe: '', LoaiDoiTuong: '', SoQuanLyHS: '', SoTien: 0, NguoiNhanThay: '', GhiChu: '', HinhThucNhan: 'Tiền mặt' });
    }
  ...
  const handleSaveAll = () => {
    if (!selectedHouseId) {
      alert("CẢNH BÁO: Bạn chưa chọn Số nhà liên kết!");
      return;
    }
    ...
    if (isEditing) {
      ...
      if (meritsList.length === 0) {
        alert("CẢNH BÁO: Vui lòng điền thông tin và nhấn 'Thêm vào danh sách' trước khi Lưu!");
        return;
      }
      onSubmit(meritsList.map(m => ({ ...m, LinkedHouseId: selectedHouseId })));
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[95vh] overflow-hidden flex flex-col border border-slate-200">
        <div className="flex items-center justify-between p-6 border-b shrink-0">
          <h2 className="text-xl font-bold text-slate-900">{isEditing ? 'Sửa hồ sơ NCC' : 'Thêm hồ sơ Người có công'}</h2>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar">
          <div className="space-y-4">
            ...
            {selectedHouse ? (
              <div className="border rounded-xl p-4 flex items-center justify-between bg-rose-50 border-rose-100 shadow-sm">
                ...
                {!isEditing && !isHouseLocked && (
                  <button onClick={() => setSelectedHouseId(undefined)} className="p-2 text-red-500 hover:bg-red-50 rounded-lg transition-colors"><Trash2 size={16} /></button>
                )}
              </div>
            ) : <div className="border-2 border-dashed border-slate-200 rounded-xl p-6 text-center bg-slate-50/50 italic text-slate-400 text-xs font-bold">Vui lòng tìm và chọn một số nhà liên kết để tiếp tục</div>}
          </div>

          {selectedHouseId && (
            <div className="space-y-6 border-t pt-6">
              <label className="text-xs font-black text-slate-400 uppercase tracking-widest flex items-center gap-2"><Heart size={14} className="text-rose-600" /> 2. Thông tin đối tượng NCC</label>
              <div className="p-6 rounded-2xl border bg-slate-50/50 border-slate-200 space-y-4 shadow-inner">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-1">
                    ...
                    <input value={currentMerit.HoTen || ''} onChange={e => setCurrentMerit({...currentMerit, HoTen: e.target.value})} className="w-full px-3 py-2 border rounded-lg text-sm bg-white" placeholder="Họ và tên" />
                  </div>
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-slate-600">Loại đối tượng <span className="text-red-500 font-black">*</span></label>
                    <select value={currentMerit.LoaiDoiTuong || ''} onChange={e => setCurrentMerit({...currentMerit, LoaiDoiTuong: e.target.value})} className="w-full px-3 py-2 border rounded-lg text-sm bg-white">
                      <option value="">-- Chọn loại đối tượng --</option>
                      {meritTypes.map(mt => <option key={mt.id} value={mt.name}>{mt.name}</option>)}
                    </select>
                  </div>
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-slate-600">Số tiền trợ cấp</label>
                    <input type="number" value={currentMerit.SoTien || 0} onChange={e => setCurrentMerit({...currentMerit, SoTien: parseFloat(e.target.value) || 0})} className="w-full px-3 py-2 border rounded-lg text-sm font-bold text-emerald-600 bg-white" />
                  </div>
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-slate-600">Người nhận thay</label>
                    <select value={currentMerit.NguoiNhanThay || ''} onChange={e => setCurrentMerit({...currentMerit, NguoiNhanThay: e.target.value})} className="w-full px-3 py-2 border rounded-lg text-sm bg-white">
                ...
                {!isEditing && (
                  <div className="flex justify-end pt-2">
                    <button type="button" onClick={handleAddToList} className={`flex items-center gap-2 px-6 py-2 rounded-xl text-xs font-black shadow-md transition-all active:scale-95 ${editingTempIndex !== null ? 'bg-indigo-600 hover:bg-indigo-700' : 'bg-slate-900 hover:bg-black'} text-white`}>
                      ...
                      {editingTempIndex !== null ? 'Cập nhật mục này' : 'Thêm vào danh sách chờ'}
                    </button>
                  </div>
                )}
              </div>
                    ...
                    <table className="w-full text-left text-xs">
                      <thead className="bg-slate-50 border-b">
                        <tr className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">
                          ...
                          <th className="px-6 py-3 text-right">Mức trợ cấp</th>
                          <th className="px-6 py-3 text-right">Thao tác</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-100">
                        {meritsList.map((m, idx) => (
                          <tr key={idx} className={`hover:bg-rose-50/30 transition-colors ${editingTempIndex === idx ? 'bg-rose-50/50' : ''}`}>
                            <td className="px-6 py-3 font-bold text-slate-700">{m.HoTen}</td>
                            <td className="px-6 py-3"><span className="text-rose-600 font-bold px-2 py-0.5 bg-rose-50 border border-rose-100 rounded">{m.LoaiDoiTuong}</span></td>
        ...
        <div className="p-6 border-t bg-slate-50 flex justify-end gap-3 shrink-0">
          ...
          <button onClick={handleSaveAll} className="px-8 py-2 bg-rose-600 text-white font-black rounded-xl shadow-lg transform active:scale-95 flex items-center gap-2 hover:bg-rose-700 transition-all"><Save size={18} /> Lưu hồ sơ</button>
        </div>
      </div>
    </div>
  );
