...
const MeritForm: React.FC<MeritFormProps> = ({ 
  initialData, onSubmit, onClose, isEditing, houseRecords, relationshipTypes, meritTypes, banks
}) => {
  ...
  const handleAddToList = () => {
    ...
    if (!isEditing) setCurrentMerit({ HoTen: '', QuanHe: '', LoaiDoiTuong: '', SoQuanLyHS: '', SoTien: 0, NguoiNhanThay: '', GhiChu: '', HinhThucNhan: 'Tiền mặt' });
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[95vh] overflow-hidden flex flex-col border border-slate-200">
                  ...
                  <div className="bg-white border rounded-xl shadow-xl overflow-hidden divide-y max-h-[350px] overflow-y-auto custom-scrollbar">
                    {filteredHouses.map(h => (
                      <button key={h.id} onClick={() => { setSelectedHouseId(h.id); setHouseSearch(''); }} className="w-full text-left px-5 py-4 hover:bg-rose-50 transition-colors flex justify-between items-center group">
                        ...
                        <div className="px-3 py-1 bg-rose-100 text-rose-700 text-[10px] font-black rounded-lg opacity-0 group-hover:opacity-100 transition-opacity uppercase">Chọn căn này</div>
                      </button>
                    ))}
                  </div>
                )}
            ...
            {selectedHouse ? (
              <div className="border-2 border-rose-500 rounded-2xl p-5 flex items-center justify-between bg-rose-50 shadow-md">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 bg-rose-600 rounded-xl flex items-center justify-center text-white shadow-lg"><Building size={24} /></div>
                  <div>
              ...
              <div className="p-6 rounded-2xl border-2 border-slate-100 bg-white space-y-4 shadow-sm">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1">
                    ...
                    <input type="number" value={currentMerit.SoTien || 0} onChange={e => setCurrentMerit({...currentMerit, SoTien: parseFloat(e.target.value) || 0})} className="w-full px-3 py-2 border rounded-lg text-sm font-black text-emerald-600 bg-slate-50" />
                  </div>
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-slate-600">Người nhận thay</label>
                    <select value={currentMerit.NguoiNhanThay || ''} onChange={e => setCurrentMerit({...currentMerit, NguoiNhanThay: e.target.value})} className="w-full px-3 py-2 border rounded-lg text-sm bg-slate-50 font-medium">
                      ...
                      <tbody className="divide-y divide-slate-100">
                        {meritsList.map((m, idx) => (
                          <tr key={idx} className={`hover:bg-rose-50/30 transition-colors ${editingTempIndex === idx ? 'bg-rose-50/50' : ''}`}>
                            ...
                            <td className="px-6 py-3 text-right font-black text-emerald-600">{(m.SoTien || 0).toLocaleString()}</td>
                            <td className="px-6 py-3 text-right">
                              <div className="flex justify-end gap-1">
                                <button onClick={() => { setEditingTempIndex(idx); setCurrentMerit(meritsList[idx]); }} className="p-1.5 text-blue-600 hover:bg-blue-100 rounded-lg transition-colors"><Edit size={14} /></button>
                                <button onClick={() => setMeritsList(prev => prev.filter((_, i) => i !== idx))} className="p-1.5 text-red-500 hover:bg-red-100 rounded-lg transition-colors"><Trash2 size={14} /></button>
                              </div>
                            </td>
                          </tr>
                        ))}
        ...
        <div className="p-6 border-t bg-slate-50 flex justify-end gap-3 shrink-0">
          ...
          <button onClick={handleSaveAll} className="px-10 py-3 bg-rose-600 text-white font-black rounded-xl shadow-xl transform active:scale-95 flex items-center gap-2 hover:bg-rose-700 transition-all text-sm"><Save size={20} /> Lưu hồ sơ</button>
        </div>
      </div>
    </div>
  );
